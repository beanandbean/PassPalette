...
- (NSArray *)match:(NSString *)password {
    ...
    while (i < password.length) {
        ...
        for (NSString *candidateName in sequences) {
            ...
            NSUInteger iPosition = [candidateSequence rangeOfString:[password substringWithRange:NSMakeRange(i, 1)]].location;
            NSUInteger jPosition = NSNotFound;
            ...
            if (iPosition != NSNotFound && jPosition != NSNotFound) {
                int direction = (int)jPosition - (int)iPosition;
                if (abs(direction) == 1) {
        ...
        if (sequence) {
            while (1) {
                NSUInteger previousPosition = NSNotFound, currentPosition = NSNotFound;
                if (j < password.length) {
                ...
                if (j == password.length || (int)currentPosition - (int)previousPosition != sequenceDirection) {
                    if (j - i > 2) {
